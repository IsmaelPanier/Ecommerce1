...
const productsInEuros = products.map(product => ({
  ...product,
  price: product.price * 0.85, // Convertir de dollars à euros (taux de change utilisé ici : 1 USD = 0.85 EUR)
}));
...
function renderProducts() {
  productList.innerHTML = productsInEuros
    .map(
      (product) => `
    <div class="product">
    ...
    <div class="product-info">
      ...
      <a class="add-to-cart" data-id="${product.id}">Ajouter au panier</a>
    </div>
  </div>
    `
    )
